...
public class Game13 {
	...
	class Food extends Pane{
		...
		public void rotate(Snake snake,Bomb bomb) {
			int[][] near_position = {{0,0},{1,0},{-1,0},{0,1},{0,-1},{2,0},{-2,0},{0,2},{0,-2},{1,1},{1,-1},{-1,1},{-1,-1}};
			...
			while(true) {
				...
				for(int[] body:snake.getPosition()) {
					for(int[] near:near_position) {
						if(body[0]+near[0]==position_x&&body[1]+near[1]==position_y) {
							food_judge=false;
							break outterLoop;							
						}
					}
				...
				for(int[] near:near_position) {
					if(bomb.getPosition()[0]+near[0]==position_x && bomb.getPosition()[1]+near[1]==position_y) {
						food_judge=false;
						break;
					}
				}
	...
	class FoodWall extends Pane{
		
		private ArrayList<int[]> foodwall_arr = new ArrayList<>();
		FoodWall(){
			int[][] near_arr= {{1,0},{0,1},{-1,0},{0,-1}};
			shuffleArray(near_arr);
			foodwall_arr.clear();
			int[] food_position= {block/2,block/2};
			for(int i=0;i<3;i++) {
				int[] tem={food_position[0]+near_arr[i][0],food_position[1]+near_arr[i][1]};
				foodwall_arr.add(tem);
			}
		}
		public ArrayList<int[]> getPostion(){
			return foodwall_arr;
		}
		public void change(Food food){
			boolean border_judge=false;
			boolean edge_judge=false;
			if((food.getPosition()[0]==0||food.getPosition()[0]==(block-1))&&(food.getPosition()[1]==0||food.getPosition()[1]==(block-1))) {
				edge_judge=true;
			}
			else if((food.getPosition()[0]==0||food.getPosition()[0]==(block-1))||(food.getPosition()[1]==0||food.getPosition()[1]==(block-1))) {
				border_judge=true;
				edge_judge=false;
			}
			
			if(!(border_judge||edge_judge)) {				
				int[][] near={{1,0},{0,1},{-1,0},{0,-1}};
				shuffleArray(near);
				foodwall_arr.clear();
				int[] food_position=food.getPosition();
				for(int i=0;i<3;i++) {
					int[] tem={food_position[0]+near[i][0],food_position[1]+near[i][1]};
					foodwall_arr.add(tem);
				}
			}
			else if(edge_judge) {
				if(food.getPosition()[0]==0&&food.getPosition()[1]==0) {
					int[][] near={{1,0},{0,1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
				...
				else if(food.getPosition()[0]==(block-1)&&food.getPosition()[1]==0) {
					int[][] near={{-1,0},{0,1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
				...
				else if(food.getPosition()[0]==0&&food.getPosition()[1]==(block-1)) {
					int[][] near={{1,0},{0,-1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
				...
				else if(food.getPosition()[0]==(block-1)&&food.getPosition()[1]==(block-1)) {
					int[][] near={{-1,0},{0,-1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
			...
			else if(border_judge){
				if(food.getPosition()[0]==0) {
					int[][] near={{1,0},{0,-1},{0,1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
					for(int i=0;i<2;i++) {
				...
				else if(food.getPosition()[0]==(block-1)) {
					int[][] near= {{-1,0},{0,1},{0,-1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
				...
				else if(food.getPosition()[1]==0) {
					int[][] near= {{1,0},{-1,0},{0,1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
					for(int i=0;i<2;i++) {
				...
				else if(food.getPosition()[1]==(block-1)) {
					int[][] near= {{1,0},{-1,0},{0,-1}};
					shuffleArray(near);
					foodwall_arr.clear();
					int[] food_position=food.getPosition();
		...
		private void add() {
	        ...
	        double centerY = getHeight() / 2;
	        getChildren().clear();
	        for(int i=0;i<foodwall_arr.size();i++) {
	        	Rectangle rect = new Rectangle(squaresize,squaresize);
	        	rect.setTranslateX(centerX-squaresize*(block/2)+squaresize*foodwall_arr.get(i)[0]);
	        	rect.setTranslateY(centerY-squaresize*(block/2)+squaresize*foodwall_arr.get(i)[1]);
	        	rect.setStroke(Color.BLACK);
		        ...
		        for(int j=0;j<div;j++) {
		        	Line line1 = new Line();
		        	line1.setStartX(rect.getTranslateX()+rect.getWidth()*j/div);
		        	line1.setEndX(rect.getTranslateX()+rect.getWidth()*(div-j)/div);
		        	line1.setStartY(rect.getTranslateY());
		        	line1.setEndY(rect.getTranslateY()+rect.getHeight());
		        	line1.setStrokeWidth(1);
		        	getChildren().add(line1);
		        	
		        	Line line2 = new Line();
		        	line2.setStartX(rect.getTranslateX());
		        	line2.setEndX(rect.getTranslateX()+rect.getWidth());
		        	line2.setStartY(rect.getTranslateY()+rect.getHeight()*j/div);
		        	line2.setEndY(rect.getTranslateY()+rect.getHeight()*(div-j)/div);
	...
	private boolean IsSnakeTouchFoodWall(Snake snake, FoodWall foodwall) {
		for(int[] body:snake.getPosition()) {
			for(int[] wall:foodwall.getPostion()) {
				if(body[0]==wall[0]&&body[1]==wall[1]) {
	...
	private FoodWall foodwall = new FoodWall();
    ...
    private static final String fileName = "SnakeGameResources/Record/Game13.txt";
    private VBox rightPane=new VBox();
    
	Game13(SettingsData settDataInput) {
		try { // 讀檔
		...
		mapPane.getChildren().add(foodwall);
	...
	private void showGG() {
		...
		GGbtn[0].setOnMouseClicked(e->{
			stage.close();
			Game13 Newgm=new Game13(settData);
			Newgm.show();
		});
	...
	public void show() {
		...
		food.setOpacity(0.3);
		foodwall.setOpacity(0.3);
		bomb.setOpacity(0.3);
		
		stage.setTitle("Game13");
	...
	private void startGame() {
        ...
        KeyFrame moveFrame = new KeyFrame(Duration.seconds(speed), event -> {
            ...
            if(IsSnakeTouchFoodWall(snake,foodwall)) {
            	showGG();
            }
