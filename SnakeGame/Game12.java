...
public class Game12 {
	...
	class Bomb extends Pane{
		...
		Bomb(){
//			int[][] temarr = {{block/3,block-6},{block/3,block+6},{block/5,block+5},{block/4,block-4},{block/6,block+5}};
			position[0]=block/3;
			position[1]=block-6;
...
//			for(int i=0;i<5;i++) {			
			...
			while(true) {
				...
				for(int[] bomb_position:position_arr) {
					if(bomb_position[0]==position_x&&bomb_position[1]==position_y) {
						bomb_judge=false;
						break;
					}
			...
			if(position_arr.size()>30) {
				position_arr.remove(0);
			}
    ...
    private static final String fileName = "SnakeGameResources/Record/Game12.txt";
    private VBox rightPane=new VBox();
    
	Game12(SettingsData settDataInput) {
		try { // 讀檔
	...
	private void showGG() {
		...
		GGbtn[0].setOnMouseClicked(e->{
			stage.close();
			Game12 Newgm=new Game12(settData);
			Newgm.show();
		});
	...
	private void startGame() {
        ...
        timelineBomb = new Timeline(new KeyFrame(Duration.seconds(2), event ->{
