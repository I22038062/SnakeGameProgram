...
//		private int[] position=new int[2];
		...
		Food(){
			int[][] tem= {{block/2,block/2},{block/3+2,block/3+7},{block/4+1,block/7+2},{block/5+6,block/8+1},{block/2+7,block/3}};
//			position[0]=block/2;
//			position[1]=block/2;
			for(int[] arr:tem) {
				position_arr.add(arr);
			}
		}
		
		public void rotate(Snake snake,Bomb bomb,int index) {
...
//			for(int i=0;i<5;i++) {	
				...
				while(true) {
					...
					for(int[] food_position:food.getPosition()) {
						if(food_position[0]==position_x&&food_position[1]==position_y) {
							food_judge=false;
						}
					}
					if(bomb.getPosition()[0]==position_x && bomb.getPosition()[1]==position_y) {
						food_judge=false;
					}
...
//			}
		...
		private void add() {
	        ...
	        double centerY = getHeight() / 2;
	        getChildren().clear();
	        for(int j=0;j<position_arr.size();j++) {
	        	
	        if(foodID==0) {
	        	Circle cir=new Circle();
		        cir.setRadius(squaresize/2*0.6);
		        cir.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]);
		        cir.setCenterY(centerY-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[1]);
		        ...
		        ell.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]-squaresize*0.1);
		        ell.setCenterY(centerY-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[1]-squaresize*0.2);
		        ...
		        rect.setStroke(Color.BLACK);
		        rect.setTranslateX(centerX-squaresize*(block/2)+squaresize*position_arr.get(j)[0]+squaresize*0.6);
		        rect.setTranslateY(centerY-squaresize*(block/2)+squaresize*position_arr.get(j)[1]+squaresize*0.1);
				...
				arc2.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]);
		        arc2.setCenterY(centerY-squaresize*(block/2-0.1)+squaresize*position_arr.get(j)[1]);
				...
				arc.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]);
		        arc.setCenterY(centerY-squaresize*(block/2-0.1)+squaresize*position_arr.get(j)[1]);
				...
				for(int i=0;i<8;i++) {
					...
					cir.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]+radi*Math.cos(theta));
			        cir.setCenterY(centerY-squaresize*(block/2-0.1)+squaresize*position_arr.get(j)[1]-radi*Math.sin(theta));
			        getChildren().add(cir);
			    }
	        }else if(foodID==2) {
	        	Ellipse ell=new Ellipse();
		        ...
		        ell.setStroke(Color.BLACK);
		        ell.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]-squaresize*0.1);
		        ell.setCenterY(centerY-squaresize*(block/2-0.36)+squaresize*position_arr.get(j)[1]-squaresize*0.2);
		        ell.setRadiusX(squaresize/3*0.6);
		        ell.setRadiusY(squaresize/3*0.3);
		        Rotate rotate = new Rotate();
		        ...
		        ell2.setCenterX(centerX-squaresize*(block/2-0.8)+squaresize*position_arr.get(j)[0]-squaresize*0.1);
		        ell2.setCenterY(centerY-squaresize*(block/2-0.36)+squaresize*position_arr.get(j)[1]-squaresize*0.2);
				...
				for(double[] iconPos:iconPosition) {
			        ...
			        cir.setCenterX(centerX-squaresize*(block/2)+squaresize*(position_arr.get(j)[0]+iconPos[0]));
			        cir.setCenterY(centerY-squaresize*(block/2)+squaresize*(position_arr.get(j)[1]+iconPos[1]));
			        cir.setStroke(Color.BLACK);
		        ...
		        for(int i=0;i<8;i++) {
		            ...
		            Circle cir=new Circle();
		        	cir.setCenterX(centerX-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[0]+squaresize/2*0.4*Math.cos(theta));
			        cir.setCenterY(centerY-squaresize*(block/2-0.5)+squaresize*position_arr.get(j)[1]+squaresize/2*0.4*Math.sin(theta));
	...
	private int IsSnakeTouchFood(Snake snake,Food food) {
		for(int[] body:snake.getPosition()) {
			for(int i=0 ;i<food.getPosition().size();i++) {	
				if(food.getPosition().get(i)[0]==body[0] && food.getPosition().get(i)[1]==body[1]) {
    ...
    private static final String fileName = "SnakeGameResources/Record/Game14.txt";
    private VBox rightPane=new VBox();
    
	Game14(SettingsData settDataInput) {
		try { // 讀檔
	...
	private void showGG() {
		...
		GGbtn[0].setOnMouseClicked(e->{
			stage.close();
			Game14 Newgm =new Game14(settData);
			Newgm.show();
		});
	...
	private void startGame() {
        ...
        KeyFrame moveFrame = new KeyFrame(Duration.seconds(speed), event -> {
            ...
            int TouchFoodresult=IsSnakeTouchFood(snake,food);
            if (TouchFoodresult!=-1) {
                snake.addlen();
                score++;
                scoreboard.setText("Score : " + score);
                if(score>highscore) {
                ...
                food.rotate(snake, bomb,TouchFoodresult);
